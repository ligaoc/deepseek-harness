/**
 * The vision bridge card's staged form over the `vision-bridge` settings
 * namespace.
 *
...
/**
 * Namespace of the vision bridge. Spelled here rather than imported: a client
 * package must not depend on a Host package.
 */
export const VISION_NS = 'vision-bridge'

/** Credential reference the bridge resolves when the section names none. */
const DEFAULT_API_KEY_REF = 'VISION_API_KEY'

/** Form field the credential control stages under. */
const API_KEY_FIELD = 'apiKey'

/** The vision bridge fields this card edits. */
export interface VisionBridgeSettings {
  ...
  model?: string
  /** Instruction sent before the image. */
  prompt?: string
}
...
/** What the vision bridge card renders. */
export interface VisionBridgeCardState extends CardShell {
...
/** The registration-side face the vision bridge card's slot entry injects. */
export interface VisionBridgeCardFace extends CardActions {
  hooks: {
    /** Card snapshot bound by the renderer as useVisionBridgeCard. */
    visionBridgeCard: SnapshotStore<VisionBridgeCardState>
  }
}

/** Bridges the `vision-bridge` scope and the credentials domain onto the card. */
export class VisionBridgeCardController {
  private readonly form: CardForm<VisionBridgeSettings>
  private readonly store: SnapshotStore<VisionBridgeCardState>
  private credential: CredentialState = { ref: '', configured: false, writable: true }

  /**
   * @param scope - the bound settings scope for the `vision-bridge` namespace.
   * @param api - wire face used for the credential the section references.
   */
  constructor(
    private readonly scope: SettingsScope<VisionBridgeSettings>,
    private readonly api: Pick<IApiClient, 'credentials'>,
  ) {
    this.form = new CardForm(
      scope,
      [textField('baseURL'), textField('model'), textField('prompt')],
  ...
  inject(): VisionBridgeCardFace {
    return { hooks: { visionBridgeCard: this.store }, ...this.form.actions() }
  }
 ...
 */
function refOf(snapshot: SettingsScopeSnapshot<VisionBridgeSettings>): string {
